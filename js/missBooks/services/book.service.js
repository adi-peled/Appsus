...
function getBooks() {
  if (!Utils.loadFromStorage('books')) {
    Utils.storeToStorage('books', gBooks)
    console.log("save")
  }
    ...
    console.log('load')
    gBooks = Utils.loadFromStorage('books')
  }
...
function getNewBooksFromAPI(bookName) {
  var prm = axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookName}`)
  return prm.then(value => value.data.items)
}

function changeFormatBook(book) {
  const newBook = {
